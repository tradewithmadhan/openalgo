...
class ZerodhaWebSocket:
    ...
    def __init__(
        ...
        # WebSocket URL — enctoken format: api_key=kitefront, access_token=enctoken+user_id
        self.ws_url = f"wss://ws.kite.trade?api_key={self.api_key}&access_token={self.access_token}&user_id={self.user_id}"
    ...
    def _refresh_access_token(self) -> bool:
        ...
        try:
            ...
            if not enctoken:
                self.logger.warning(
                    "No enctoken found on reconnect — keeping existing token"
                )
                return False
            with self.lock:
                changed = enctoken != self.access_token
            ...
            self.logger.info("Refreshed enctoken from database for reconnect")
            return changed
        except Exception as e:
            self.logger.error(f"Error refreshing enctoken on reconnect: {e}")
            return False
