...
class ZerodhaenctokenWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    ...
    def __init__(self):
        ...
        super().__init__()
        self.logger = get_logger("zerodhaenctoken_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "zerodhaenctoken"
        self.running = False
    ...
    def initialize(
        ...
        try:
            ...
            # Get enctoken from database (feed_token column)
            ...
            # Fetch Zerodha user_id from profile API
            import httpx
            try:
                resp = httpx.get(
                ...
                if profile.get("status") == "success":
                    self.zerodha_user_id = profile["data"]["user_id"]
                    self.logger.info(f"Fetched Zerodha user_id: {self.zerodha_user_id}")
                else:
                    return {"status": "error", "message": f"Failed to fetch profile: {profile.get('message')}"}
            except Exception as e:
                return {"status": "error", "message": f"Failed to fetch Zerodha user_id: {e}"}

            # Initialize WebSocket client with enctoken format
            # URL: wss://ws.kite.trade?api_key=kitefront&access_token={enctoken}&user_id={zerodha_user_id}
            ...
            self.logger.info(f"Zerodha adapter initialized for user {user_id} (Zerodha ID: {self.zerodha_user_id})")
